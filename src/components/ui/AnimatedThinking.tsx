...
function AnimatedThinking({
  ...
  const questionPathsRef = useRef<SVGGElement>(null);
  const personPathsRef = useRef<SVGGElement>(null);

  useEffect(() => {
    if (!svgRef.current || !questionPathsRef.current || !personPathsRef.current) return;

    const questionGroup = questionPathsRef.current;
    const personGroup = personPathsRef.current;

    const ctx = gsap.context(() => {
      // Get all paths in both groups
      const questionPaths = Array.from(questionGroup.querySelectorAll("path"));
      const personPaths = Array.from(personGroup.querySelectorAll("path"));

      if (personPaths.length === 0 || questionPaths.length === 0) return;

      // Set initial states for stroke drawing animation
      ...
      // Setup all paths
      personPaths.forEach(setupPath);
      questionPaths.forEach(setupPath);

      // Animation timeline
      const tl = gsap.timeline({ delay: 0.3 });

      // Animate person paths first (draw stroke then fill)
      personPaths.forEach((path, i) => {
        tl.to(
          path,
          {
            strokeDashoffset: 0,
      ...
      // Animate question mark paths (draw then fill)
      ...
      // Question mark floating animation (continuous)
      tl.to(
        questionGroup,
        {
          y: -3,
...
export default memo(AnimatedThinking);
